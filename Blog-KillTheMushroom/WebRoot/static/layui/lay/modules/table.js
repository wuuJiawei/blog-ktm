...
function(e) {
    ...
    A = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" \
         style="\
         	...
         	{{# if(d.data.height){ }}\
         		{{# console.log(d); }}\
         		{{# var class_wjw = document.querySelector("."+d.VIEW_CLASS); }}\
		        {{# var height_wjw = d.data.height + ""; }}\
		        {{# if(height_wjw.indexOf("%")!=-1){ }}\
	         		height:{{d.data.height}};\
	        	{{# }else{ }}\
	         		height:{{d.data.height}}px;\
         		{{# } }}\
		 	{{# } }}">', "{{# var fixed, right; }}", '<div class="layui-table-header">', z(), "</div>", '<div class="layui-table-body layui-table-main">', T, "</div>", '{{# if(fixed && fixed !== "right"){ }}', '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', z({
        fixed: !0
