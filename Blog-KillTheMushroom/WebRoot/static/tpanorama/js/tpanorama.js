(function (global, undefined) {
    ...
    panoramaSetting.prototype = {
        ...
        init: function () {
            ...
            setTimeout(function () {
                adptpImg(that.def.width, that.def.height, that.def.imgUrl);
                clearCanvas();
    ...
    function adptpImg(width, height, url) {
        if (!isEmpty(width)) {
            _setContainer.style.width = width;
        }
        if (!isEmpty(height)) {
            _setContainer.style.height = height;
        }else{
        	height = scale * _setContainer.style.width.split("px")[0];
        }
