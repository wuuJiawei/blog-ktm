// 图标库： http://zavoloklom.github.io/material-design-iconic-font/v1/

var click = device.mobile() ? 'touchstart' : 'click';
$(function() {
	//顶部菜单栏缩放
	$(document).on(click, '#swap-vertical', function(){
		$('#sidebar').height('100%')
		$('header,#sidebar,#main').animate({'margin-top':"-70px"});
		$('.menu-vertical').animate({'margin-top':"0px"});
		$('.content_main').height('100%');
	});
	$(document).on(click, '.menu-vertical', function(){
		var windowHeight = $(window).height();
		$('.menu-vertical').animate({'margin-top':"-70px"});
		$('header,#sidebar,#main').animate({'margin-top':"0px"});
		$('.content_main').height(windowHeight-70-48);
		$('#sidebar').height(windowHeight-70);
	});
	// 侧边栏操作按钮
